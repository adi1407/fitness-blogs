...
const SUB_AFTER =
  "Cinematic framing fully realized across spatial coordinates.";
...
function useResponsive() {
  ...
  useEffect(() => {
    ...
    const read = () =>
      setR(mq.matches ? RESPONSIVE_SMALL : RESPONSIVE_DESKTOP);
    read();
...
function StackSpreadStage({
  ...
  const beforeOpacity = useTransform(
    progress,
    [textFadeStart, textFadeStart + 0.2, 0.65, 0.75],
    [0, 1, 1, 0],
  );
  const beforeY = useTransform(progress, [textFadeStart, textFadeStart + 0.2], [
    20,
    0,
  ]);
