...
export function ProteinCalculatorForm() {
  ...
  const result = useMemo(() => {
    ...
    return { kg, grams, low: Math.round(kg * (factor - 0.2)), high: Math.round(kg * (factor + 0.2)) };
  }, [weight, unit, goal]);
    ...
    <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
        ...
        <label className="block">
          ...
          <div className="mt-2 flex gap-2">
            <input
              ...
              className="w-full rounded-xl border border-border bg-white px-3 py-2 text-foreground outline-none focus:border-primary"
            />
            <select
              value={unit}
      ...
      <div className="mt-8 rounded-xl bg-brand-50 p-5">
        ...
        <p className="mt-1 text-4xl font-semibold tracking-tight text-foreground">
          {result.grams}
          <span className="ml-2 text-lg font-medium text-muted-foreground">g/day</span>
        </p>
        <p className="mt-2 text-sm text-muted-foreground">
          Practical range ~{result.low}–{result.high} g based on ~{result.kg.toFixed(1)} kg
          body weight. Educational estimate only.
        </p>
      </div>
